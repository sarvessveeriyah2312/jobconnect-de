...
export class SidebarComponent {
  ...
  getRoleLabel(): string {
    const role = this.userRole();
    return ROLE_PERMISSIONS[role]?.label || 'Guest';
  }

  getPermissionSummary(): string {
    const role = this.userRole();
    const roleConfig = ROLE_PERMISSIONS[role];
    if (!roleConfig) return 'Limited access';
    const count = roleConfig.permissions.length;
    return `${count} resource${count !== 1 ? 's' : ''} accessible`;
  }
